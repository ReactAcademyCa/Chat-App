...
const PORT = process.env.PORT || 3000; // take port from heroku or for localhost
const app = express(); // express app which is used boilerplate for HTTP
const service = http.Server(app);

let clientInfo = {};

//socket io module
let socketSvc = socket(service);

// expose the folder via express
...
// socketSvc.on listens for events
socketSvc.on('connection', function(socket) {
